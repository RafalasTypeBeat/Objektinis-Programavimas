...
double galutinis(double egz, const deque<double>& hw, double (*kriterijus)(deque<double>))
{
    if(hw.size() == 0)
...
void generate(int &FileSkc)
{
    ...
    //deque<mokinys>::iterator it = mok.begin();
    int MokiniuSkc = 100;
    FileSkc = 0;
...
void sortByGrades(deque<mokinys>& mok, deque<mokinys> &fail, deque<mokinys> &pass){
    deque<mokinys>::iterator it = mok.end();
    it--;
    while(it != mok.begin()){
...
void readAndSort(int FileSkc)//nuskaito is sugeneruotu failu ir sudeda i dequeiu
{
    auto startRead = hrClock::now();
    deque<mokinys> mok, fail, pass;
    mokinys temp;
...
void writeSorted(deque<mokinys> &fail,deque<mokinys> &pass, int FileNr)
{
    ...
    stringstream nameFail, namePass;
        ...
        deque<mokinys>::iterator itP = pass.begin(); //islaike
        deque<mokinys>::iterator itF = fail.begin(); //neislaike
        while(itP != pass.end()){
            stringstream name, lastname, output;
            name << itP->name << " ";
